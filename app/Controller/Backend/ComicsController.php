...
class ComicsController extends BaseBackendController
{
    ...
    public function initData()
    {
        ...
        $this->view->setVar('tagArr', $this->comicsRepo->getGroupAttrAll());
    }
    ...
    {
        if ($this->isPost()) {
            $result = $this->comicsRepo->getList($_REQUEST);
    ...
    public function detailAction()
    {
        ...
        if (!empty($id)) {
            $result = $this->comicsRepo->getDetail($id);
            $this->view->setVar('row', $result);
            $this->view->setVar('chapterList',$this->comicsRepo->getChapterList($id));
        }
        $this->initData();
    }
    ...
    public function chapterDetailAction()
    {
        ...
        $chapter = $this->comicsRepo->getChapterDetail($id);
        if(empty($chapter)){
            $this->sendErrorResult('章节错误!');
        }
    ...
    {
        $result = $this->comicsRepo->save($_POST);
        if ($result) {
    ...
    public function doAction()
    {
        ...
        foreach ($ids as $id) {
            if (!empty($update)) {
                $update['_id'] = $id;
                $this->comicsRepo->comicsService->save($update);
            }
            $this->comicsRepo->asyncEs($id);
    ...
    public function asyncAction()
    {
        ...
        $ids = explode("\n", $ids);
        foreach ($ids as $id) {
            $this->comicsRepo->asyncMrs(trim($id));
        }
        $this->sendSuccessResult();
    }
}