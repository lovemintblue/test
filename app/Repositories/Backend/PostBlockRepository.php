...
class PostBlockRepository extends BaseRepository
{
    ...
    public function getList($request)
    {
        ...
        $fields = array();
        $count  = $this->postBlockService->count($query);
        $items  = $this->postBlockService->getList($query, $fields, array($sort => $order), $skip, $pageSize);
        foreach ($items as $index => $item) {
            $item['created_at']     = dateFormat($item['created_at']);
            $item['updated_at']     = dateFormat($item['updated_at']);
            $item['is_disabled']    = CommonValues::getIsDisabled($item['is_disabled']);
            $item['style']          = CommonValues::getMovieBlockStyles($item['style']);
            $item['position']       = $this->getPosition($item['position'])?:'-';
            $item['is_ai_text']     = CommonValues::getIsAi($item['is_ai']*1);
            $items[$index] = $item;
        }
    ...
    public function save($data)
    {
        $row = array(
            ...
            'style'         => $this->getRequest($data, 'style','int',0),
            ...
            'is_ai'         => $this->getRequest($data,'is_ai','int',0)
        );
        ...
        return $this->postBlockService->save($row);
    }
    ...
    {
        $row = $this->postBlockService->findByID($id);
        if (empty($row)) {
    ...
    {
        return $this->postBlockService->delete($id);
    }
