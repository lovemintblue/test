...
class NovelTagRepository extends BaseRepository
{
    ...
    public function getList($request)
    {
        ...
        $fields = array();
        $count  = $this->novelTagService->count($query);
        $items  = $this->novelTagService->getList($query, $fields, array($sort => $order), $skip, $pageSize);
    ...
    public function save($data)
    {
        ...
        $checkRow=$this->novelTagService->findFirst($where);
        if ($checkRow) {
            throw  new BusinessException(StatusCode::PARAMETER_ERROR, '分类名称不能重复!');
        }
        return $this->novelTagService->save($row);
    }
    ...
    {
        $row = $this->novelTagService->findByID($id);
        if (empty($row)) {
    ...
    {
        return $this->novelTagService->delete($id);
    }
