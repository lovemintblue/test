...
use App\Services\CartoonCategoryService;
...
class MovieCategoryRepository extends BaseRepository
{
    ...
    public function getList($request)
    {
        ...
        $fields = array();
        $count  = $this->movieCategoryService->count($query);
        $items  = $this->movieCategoryService->getList($query, $fields, array($sort => $order), $skip, $pageSize);
    ...
    public function save($data)
    {
        ...
        $checkRow=$this->movieCategoryService->findFirst($where);
        if ($checkRow) {
            throw  new BusinessException(StatusCode::PARAMETER_ERROR, '分类名称不能重复!');
        }
        return $this->movieCategoryService->save($row);
    }
    ...
    {
        $row = $this->movieCategoryService->findByID($id);
        if (empty($row)) {
    ...
    {
        return $this->movieCategoryService->delete($id);
    }
    ...
    {
        return $this->movieCategoryService->getAll();
    }
