<?php

declare(strict_types=1);

namespace App\Repositories\Backend;

use App\Constants\CommonValues;
use App\Constants\StatusCode;
use App\Core\Repositories\BaseRepository;
use App\Exception\BusinessException;
use App\Services\ArticleCategoryService;
use App\Services\ArticleService;

/**
 * 文章
 * @package App\Repositories\Backend
 *
 * @property  ArticleService $articleService
 * @property  ArticleCategoryService $articleCategoryService
 */
class ArticleRepository extends BaseRepository
{

    /**
     * 获取列表
     * @param $request
     * @return array
     */
    public function getList($request)
    {

        $page = $this->getRequest($request, 'page', 'int', 1);
        $pageSize = $this->getRequest($request, 'pageSize', 'int', 15);
        $sort     = $this->getRequest($request, 'sort', 'string', '_id');
        $order    = $this->getRequest($request, 'order', 'int', -1);
        $query = array();
        $filter = array();

        if ($request['name']) {
            $filter['name'] = $this->getRequest($request, 'name');
            $query['name'] = array('$regex' => $filter['name'], '$options' => 'i');
        }
        if ($request['category_code']) {
            $filter['category_code'] = $this->getRequest($request, 'category_code');
            $query['category_code'] = $filter['category_code'];
        }

        $skip = ($page - 1) * $pageSize;
        $fields = array(
            '_id', 'title', 'created_at', 'updated_at', 'category_code', 'is_recommend', 'click', 'sort', 'img', 'show_dialog'
        );
        $count = $this->articleService->count($query);
        $items = $this->articleService->getList($query, $fields, array($sort => $order), $skip, $pageSize);
        $articleCategories = $this->articleCategoryService->getAll();

        foreach ($items as $index => $item) {
            $item['created_at'] = dateFormat($item['created_at']);
            $item['updated_at'] = dateFormat($item['updated_at']);
            $item['is_recommend_text'] = CommonValues::getIsRecommend($item['is_recommend']);
            $item['category_name'] = $articleCategories[$item['category_code']]['name'];
            $items[$index] = $item;
        }

        return array(
            'filter' => $filter,
            'items' => empty($items) ? array() : array_values($items),
            'count' => $count,
            'page' => $page,
            'pageSize' => $pageSize
        );
    }

    /**
     * 保存数据
     * @param $data
     * @return bool|int|mixed
     * @throws BusinessException
     */
    public function save($data)
    {
        $row = array(
            'title' => $this->getRequest($data, 'title'),
            'content' => $this->getRequest($data, 'content'),
            'img' => $this->getRequest($data, 'img'),
            'seo_keywords' => $this->getRequest($data, 'seo_keywords'),
            'seo_description' => $this->getRequest($data, 'seo_description'),
            'url' => $this->getRequest($data, 'url'),
            'category_code' => $this->getRequest($data, 'category_code'),
            'is_recommend' => $this->getRequest($data, 'is_recommend', 'int', 0),
            'sort' => $this->getRequest($data, 'sort', 'int', 0),
            'click' => $this->getRequest($data, 'click', 'int', 0),
        );
        if (empty($row['title']) || empty($row['category_code']) || empty($row['content'])) {
            throw  new BusinessException(StatusCode::PARAMETER_ERROR, '参数错误!');
        }
        if ($data['_id'] > 0) {
            $row['_id'] = $this->getRequest($data, '_id', 'int');
        }
        return $this->articleService->save($row);
    }

    /**
     * 获取详情
     * @param $id
     * @return mixed
     * @throws BusinessException
     */
    public function getDetail($id)
    {
        $row = $this->articleService->findByID($id);
        if (empty($row)) {
            throw  new BusinessException(StatusCode::DATA_ERROR, '数据不存在!');
        }
        return $row;
    }

    /**
     * 删除订单
     * @param $id
     * @return mixed
     */
    public function delete($id)
    {
        return $this->articleService->delete($id);
    }


}