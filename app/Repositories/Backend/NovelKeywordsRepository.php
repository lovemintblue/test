...
class NovelKeywordsRepository extends BaseRepository
{
    ...
    public function getList($request)
    {
        ...
        $fields = array();
        $count  = $this->novelKeywordsService->count($query);
        $items  = $this->novelKeywordsService->getList($query, $fields, array($sort => $order), $skip, $pageSize);
    ...
    {
        $row = $this->novelKeywordsService->findByID($id);
        if (empty($row)) {
    ...
    public function save($data)
    {
        ...
        if ($this->novelKeywordsService->count(['_id'=>$row['_id']])) {
            return $this->novelKeywordsService->save($row);
        }else{
            return $this->novelKeywordsService->novelKeywordsModel->insert($row);
        }
    }
    ...
    {
        return $this->novelKeywordsService->save($data);
    }
    ...
    {
        return $this->novelKeywordsService->delete($id);
    }
