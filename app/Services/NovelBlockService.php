...
use App\Models\NovelBlockModel;

/**
 *  小说模块
 * @package App\Services
 *
 * @property  NovelBlockModel $novelBlockModel
...
class NovelBlockService extends BaseService
{
    ...
    {
        return $this->novelBlockModel->find($query, $fields, $sort, $skip, $limit);
    }
    ...
    {
        return $this->novelBlockModel->count($query);
    }
    ...
    {
        return $this->novelBlockModel->findFirst($query, $fields);
    }
    ...
    {
        return $this->novelBlockModel->findByID(intval($id));
    }
    ...
    {
        if ($data['_id']) {
            $result = $this->novelBlockModel->update($data, array("_id" => $data['_id']));
        } else {
            $result = $this->novelBlockModel->insert($data);
        }
        return $result;
    }
    ...
    {
        $result = $this->novelBlockModel->delete(array('_id' => intval($id)));
        return $result;
    }
    ...
    {
        $keyName='novel_block_'.$page.'_'.$position;
        ...
        if (is_null($result)) {
            ...
            $items = $this->novelBlockModel->find($query, array(), array('sort' => -1), $skip, $pageSize);
    ...
    public function doSearch($query)
    {
        ...
        $items = $this->novelBlockModel->find($filter,array(),array('sort'=>-1),($page-1)*$pageSize,$pageSize);
        $result = array();
        foreach ($items as $item) {
            $result[] = array(
                'id' => strval($item['_id']),
