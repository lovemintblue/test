...
use App\Models\ComicsChapterOrderModel;
use App\Models\ComicsModel;
use App\Models\ComicsOrderModel;
 ...
 * @property  ComicsChapterOrderModel $comicsChapterOrderModel
 * @property  ComicsOrderModel $comicsOrderModel
...
class ComicsService extends BaseService
{
    ...
    {
        return $this->comicsModel->find($query, $fields, $sort, $skip, $limit);
    }
    ...
    {
        return $this->comicsModel->count($query);
    }
    ...
    {
        return $this->comicsModel->findFirst($query, $fields);
    }
    ...
    {
        return $this->comicsModel->findByID($id);
    }
    ...
    {
        if ($data['_id']) {
            return $this->comicsModel->update($data, array("_id" => $data['_id']));
        } else {
            return $this->comicsModel->insert($data);
        }
    }
    ...
    {
        $result = $this->comicsModel->delete(array('_id' => $id));
        if ($result) {
            $this->elasticService->delete('comics', 'comics', $id);
        }
        return $result;
    }
    ...
    {
        return $this->comicsModel->updateRaw($document, $where);
    }
    ...
    public function asyncMrs($query)
    {
        ...
        if($config->mrs->source=='laosiji'){//老司机库
            $result = $this->mrsSystemService->getLsjComicsDetail(['id'=>$query['ids']]);
            $items = $result?[$result]:[];
        }else{
            $query['info_status'] = 1;
            $items = $this->mrsSystemService->getComicsList($query);
        }
        foreach ($items as $item) {
            $info['count'] += 1;
            if ($this->saveMrsData($item)) {
                LogUtil::info('Async comics ok:' . $item['name']);
                $info['success'] += 1;
            } else {
                LogUtil::error('Async comics error:' . $item['name']);
            }
        }
        return $info;
    }
    ...
    public function saveMrsData($mrsData, $isPublic = false)
    {
        ...
        if (!$isPublic) {
            if ($mrsData['tags']) {
                foreach ($mrsData['tags'] as $tag) {
                    ...
                    $tagModel = $this->comicsTagService->findFirst(['_id' => $tag['id']]);
                    if (empty($tagModel)) {
                        $tags[] = $this->comicsTagService->insert([
                            '_id' => $tag['id'] * 1,
                            'name' => $tag['name'],
                            'is_hot' => 0,
                            'attribute' => $tag['group'] ?: ''
                        ...
                        if (empty($tagModel['attribute']) && !empty($tag['group'])) {
                            $this->comicsTagService->save(array(
                                '_id' => $tagModel['_id'],
                                'attribute' => $tag['group']
                            ));
                        }
        ...
        $mid = $mrsData['id'];
        $comicsModel = $this->findFirst(['_id' => $mid]);
        if (empty($comicsModel)) {
            ...
            $this->comicsModel->insert($comicsSaveData);
        } else {
...
//                'tags' => $tags,
            ...
            $this->comicsModel->update($comicsSaveData, array('_id' => $mid));
        }
        foreach ($mrsData['chapter'] as $chapter) {
            $chapterId = $chapter['id'];
            $chapter['created_at'] = $chapter['created_at']?strtotime($chapter['created_at']):time();
            $checkItem = $this->comicsChapterModel->findFirst(array('_id' => $chapterId), array('_id', 'updated_at'));
            unset($chapter['id']);
            $chapter['updated_at'] = $chapter['updated_at']?strtotime($chapter['updated_at']):time();
            $chapter['sort'] = intval($chapter['sort']);
            if ($checkItem) {
                if ($checkItem['updated_at'] != $chapter['updated_at'] || $checkItem['created_at'] != $chapter['created_at']) {
                    $this->comicsChapterModel->update($chapter, array('_id' => $chapterId));
                }
                continue;
            }
            $chapter['_id'] = $chapterId;
            $chapter['comics_id'] = $mid;
            $this->comicsChapterModel->insert($chapter);
        }
        return true;
    }
    ...
    public function doSearch($userId, $filter = [])
    {
        ...
        if ($keyword) {
            ...
            $query['min_score'] = '1.2';
            $this->comicsKeywordsService->do($keyword);
        }
        if (!empty($catId)) {
            $catCode = CommonValues::getComicsCategoriesCode($catId);
            array_push($query['query']['bool']['must'], ['term' => ['cat_code' => $catCode]]);
            unset($catId);
        }
        if (!empty($tagId)) {
        ...
        if(!empty($updateDate)){
            $updateDate  = CommonValues::getWeek($updateDate);
            array_push($query['query']['bool']['must'], ['term' => ['update_date_value' =>$updateDate]]);
            unset($updateDate);
        }
        if (!empty($ids)) {
            $idArr = explode(',', $ids);
        ...
        $items = array();
        $result = $this->elasticService->search($query, 'comics', 'comics');
        //获取计数器
        $redisCounterKeys = [];
        foreach ($result->hits->hits as $item) {
            ...
            $redisCounterKeys[] = "comics_comment_".$id;;
        }
        ...
        foreach ($result->hits->hits as $item) {
            ...
            $item = [
                ...
                'comment' => value(function () use ($item,$counterMap) {
                    $keyName = 'comics_comment_' . $item->id;
                    $real = $counterMap[$keyName] ?? 0;
    ...
    public function asyncEs($id)
    {
        ...
        $row['tags'] = [];
        //只储存一级标签
        $tagArr = $this->comicsTagService->getAll();
        ...
        if ($updated) {
            $this->comicsModel->updateRaw(array('$set' => $updated), array('_id' => $id));
        }

        $row['last_update'] = intval($row['last_update']);
        $row['show_at'] = intval($row['show_at']);
        $row['update_date_value'] = empty($row['update_date'])?0:CommonValues::getWeek($row['update_date'])*1;

        $this->commonService->setRedisCounter("comics_click_" . $row['id'], $row['real_click']);
        $this->commonService->setRedisCounter("comics_favorite_" . $row['id'], $row['real_favorite']);
        $this->commonService->setRedisCounter("comics_comment_" . $row['id'], $realComment);

        unset($row['_id']);
        return $this->elasticService->save($row['id'], $row, 'comics', 'comics');
    }
    ...
    {
        $comicsId = $data['comics_id'];
        switch ($data['action']) {
            case 'click':
                $this->commonService->updateRedisCounter("comics_click_{$comicsId}", 1);
                $this->updateRaw(array('$inc' => array('real_click' => 1)), array('_id' => $comicsId));
                $this->analysisCartoonService->inc($comicsId, 'click', 1);
                break;
            case 'buy':
                $this->updateRaw(array('$inc' => array('buy' => 1)), array('_id' => $comicsId));
                $this->analysisCartoonService->inc($comicsId, 'buy_num', 1);
                break;
            case 'favorite':
                $this->commonService->updateRedisCounter("comics_favorite_{$comicsId}", 1);
                $this->updateRaw(array('$inc' => array('real_favorite' => 1)), array('_id' => $comicsId));
                $this->analysisCartoonService->inc($comicsId, 'favorite', 1);
                break;
            case 'unFavorite':
                $this->commonService->updateRedisCounter("comics_favorite_{$comicsId}", -1);
                $this->updateRaw(array('$inc' => array('real_favorite' => -1)), array('_id' => $comicsId));
     ...
     * @return mixed
     */
    public function setFavoriteRate($id, $rate)
    {
        return $this->updateRaw(['$set' => ['favorite_rate' => intval($rate)]], ['_id' => $id]);
    }
     ...
     */
    public function getDetail($comicsId)
    {
        $keyName = "comics_detail_{$comicsId}";
        $result = getCache($keyName);
        if (empty($result)) {
            $result = $this->doSearch(null, array("ids" => $comicsId));
     ...
     * @return array
     */
    public function getChapterList($comicsId)
    {
        if (empty($comicsId)) {
            return [];
        }
        $cacheKey = 'comics_chapter_list_' . $comicsId;
        $result = getCache($cacheKey);
        if ($result === null) {
            $result = $this->comicsChapterModel->find(
                array('comics_id' => $comicsId),
                array('_id', 'name', 'img'),
    ...
    {
        $cacheKey = 'comics_chapter_detail_' . $id;
        $result = getCache($cacheKey);
        if ($result === null) {
            $result = $this->comicsChapterModel->findByID($id);
            setCache($cacheKey, $result, mt_rand(180, 240));
        }
        return $result;
    }
    ...
    public function hasBuyChapter($chapterId,$userId)
    {
        ...
        $result =$this->comicsChapterOrderModel->count(array('_id'=>$orderId));
     ...
     * @return bool
     */
    public function buyChapter($comicsId,$chapterId,$userId,$orderSn,$money)
    {
        $orderId = md5($chapterId.'_'.$userId);
        $data = array(
            '_id' => $orderId,
        ...
        $this->comicsChapterOrderModel->insert($data);
        $orderId = md5($comicsId.'_'.$userId);
        $count = $this->comicsOrderModel->count(array('_id'=>$orderId));
        if($count<1){
            $data = array(
                '_id' => $orderId,
                'comics_id'=>$comicsId,
            ...
            $this->comicsOrderModel->insert($data);
        }
        return true;
    }
    ...
    public function getBuyLogs($userId, $page = 1, $pageSize = 20)
    {
        ...
        $items = $this->comicsOrderModel->find(['user_id' => intval($userId)], [], ['created_at' => -1], $skip, $pageSize);
        foreach ($items as $item) {
